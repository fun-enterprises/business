...
You may also customize the root directory in config file via [`srcDir`](/es/config/app-configs#srcdir) option too. Running `vitepress dev` with the following setting acts same as running `vitepress dev docs` command.
...
```md
<!-- Do -->
[Getting Started](/es/guide/getting-started)
[Getting Started](../es/guide/getting-started)

<!-- Don't -->
[Getting Started](/es/guide/getting-started.md)
[Getting Started](/es/guide/getting-started.html)
```
...
However, you may also generate a clean URL by setting up [`cleanUrls`](/es/config/app-configs#cleanurls) option.
...
You may configure the mapping via [`rewrites`](/es/config/app-configs#rewrites) option like this.
